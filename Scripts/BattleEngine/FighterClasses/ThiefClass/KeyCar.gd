...
func use_key_car_attack(move_info, success_roll, crit_roll):
...
func _on_key_car_complete(_animation_name):
	...
	current_user.rotate_to(original_rotation, .2, _on_key_car_return)

	current_user.animation_player.play("ram_return")

func _on_key_car_return():
	current_user.animation_player.animation_finished.disconnect(
		_on_key_car_return
	)
